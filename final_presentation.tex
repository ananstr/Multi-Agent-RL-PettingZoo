...
\begin{frame}{Sequential Social Dilemmas}
    ...
    \begin{columns}
        ...
        \begin{column}{0.4\textwidth}
            ...
            autoresume, % Resumes after page focus returns
            autopause, % Pauses when page loses focus
            width=\linewidth]{20}{latex/imgs/oai-chaser/frame_}{0}{436}
...
\begin{frame}{Experimental Setup: Simple Adversary}
    
    \begin{columns}
        \begin{column}{0.5\textwidth}
            \textbf{Environment: Simple Adversary}
            \begin{itemize}
                \item 2 good agents, 1 adversary
                \item Goal for the good agents: Reach landmarks, avoid capture
                \item Goal for the adversary: Capture good agents
                \item Zero-sum competition
            \end{itemize}
        \end{column}
        \begin{column}{0.5\textwidth}
            \textbf{Good Agents Learn:}
            \begin{itemize}
                \item Sophisticated movement to avoid capture
                \item Strategic target selection  
                \item Time movements to reduce exposure
            \end{itemize}
            
            \vfill
            \textbf{Adversary Agents Learn:}
            \begin{itemize}
                \item Predicting good agent movements
                \item Coordinated chasing behaviors
                \item Driving agents toward capture zones
                \item Preventing access to landmarks
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
